-- Создание таблицы для B2B API ключей
...
CREATE TABLE IF NOT EXISTS b2b_api_sessions (
    id SERIAL PRIMARY KEY,
    api_key_id INTEGER REFERENCES b2b_api_keys(id),
...
CREATE TABLE IF NOT EXISTS b2b_usage_stats (
    id SERIAL PRIMARY KEY,
    api_key_id INTEGER REFERENCES b2b_api_keys(id),
    date DATE NOT NULL,
    bytes_transferred BIGINT DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_b2b_api_keys_key_id ON b2b_api_keys(key_id);
CREATE INDEX IF NOT EXISTS idx_b2b_api_keys_tier ON b2b_api_keys(tier);
CREATE INDEX IF NOT EXISTS idx_b2b_api_keys_active ON b2b_api_keys(is_active);
CREATE INDEX IF NOT EXISTS idx_b2b_api_sessions_api_key ON b2b_api_sessions(api_key_id);
CREATE INDEX IF NOT EXISTS idx_b2b_api_sessions_active ON b2b_api_sessions(is_active);
CREATE INDEX IF NOT EXISTS idx_b2b_usage_stats_date ON b2b_usage_stats(date);
CREATE INDEX IF NOT EXISTS idx_b2b_usage_stats_api_key ON b2b_usage_stats(api_key_id);
