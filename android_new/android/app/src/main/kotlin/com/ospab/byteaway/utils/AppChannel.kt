...
import com.ospab.byteaway.service.ByteAwayVpnService
...
object AppChannel {
                    ...
                    "getInstalledApps" -> {
                        Thread {
                            try {
                                ...
                                context.mainExecutor.execute { result.success(apps) }
                            } catch (e: Exception) {
                                context.mainExecutor.execute { result.error("ERROR", e.message, null) }
                            }
                        }.start()
                    }
                    "getExcludedApps" -> {
                        val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
                        result.success(prefs.getStringSet(PREFS_EXCLUDED, emptySet())?.toList())
                    }
                    "addExclude" -> {
                        val pkg = call.argument<String>("pkg")
                        if (pkg != null) {
                            sendIntentToService(context, ByteAwayVpnService.ACTION_ADD_EXCLUDE, pkg)
                            result.success(null)
                        } else {
                            result.error("INVALID_ARGUMENT", "Package name missing", null)
                        }
                    }
                    "removeExclude" -> {
                        val pkg = call.argument<String>("pkg")
                        if (pkg != null) {
                            sendIntentToService(context, ByteAwayVpnService.ACTION_REMOVE_EXCLUDE, pkg)
                            result.success(null)
                        } else {
                            result.error("INVALID_ARGUMENT", "Package name missing", null)
                        }
    ...
    private fun sendIntentToService(context: Context, action: String, pkg: String) {
        val intent = Intent(context, ByteAwayVpnService::class.java).apply {
            this.action = action
            putExtra(ByteAwayVpnService.EXTRA_EXCLUDE_PKG, pkg)
        }
